...
- **已打包**：`LocalMind.exe`（免安装）+ `LocalMindSetup.exe`（NSIS），均为干净正式版
...
- 本机工具链：VS Build Tools（`E:\Visual Studio`）+ Rust MSVC + Node（`E:\APP`）+ Python 3.11（`E:\APP\python.exe`）
- **LIBCLANG_PATH** 必须指向 `E:\APP\Lib\site-packages\clang\native`（llama-cpp-sys 的 bindgen 需要 libclang.dll）
- **cmake** 不在 PATH，用 CLion 自带：`E:\APP\CLion 2026.2.1\bin\cmake\win\x64\bin` 加进 PATH
- 构建命令：`node node_modules/@tauri-apps/cli/tauri.js build`（node 用绝对路径；PATH 需含 `E:\APP`）
- agent 重新打包：`E:\code\LocalAITools\localmind\scripts\build\agent-venv\Scripts\python.exe localmind\scripts\pack_agent_exe.py`（venv 只装 pydantic-ai-slim[openai] + pyinstaller）
